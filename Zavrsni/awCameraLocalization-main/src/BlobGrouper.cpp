    ...
    if(this->rawPicture != nullptr){
        ...
        for(int i = 0; i < this->blobs.size(); i++){
            ...
            if(this->isCenterHovered){
                ...
                if(this->blobs[i].middle.x < 640 && this->blobs[i].middle.y < 480 && this->blobs[i].middle.x > 0 && this->blobs[i].middle.y > 0){
        ...
        ImGui::Image((void*)(intptr_t)this->texture, ImVec2(640*GlobalParams::getInstance().getZoom().scaleFactor, 480*GlobalParams::getInstance().getZoom().scaleFactor));
    }
